...
echo "[" . date("d-m-y H:i:s") . "] ", "VTR CULTURA scraper start!", PHP_EOL;

// scraper vars
$config = json_decode(file_get_contents("vtr-cl.ext.CULTURA.json"));
$dict_fechas = array(
  strtotime("today"),
  strtotime("tomorrow"),
  strtotime("2 days")
...
$dict_canales = array(
  "518993217" => "cl.ext.13c"
);

$grilla_url = "http://televisionvtr.cl/index.php?obt=grillacat&canal_tipo=cultura&fecha=%FECHA%&comuna=Santiago&canal_cantidad=20&_=" . time();
$grilla_fields = "%FECHA%";

$detalles_url = "http://televisionvtr.cl/index.php?obt=minificha&channels=%CHANNELID%&programs=%PROGRAMID%&starttime=%STARTTIME%&startdate=%STARTDATE%&canal_tipo=Cultural&_=" . time();
...
echo "[" . date("d-m-y H:i:s") . "] ", "VTR CULTURA scraper done!", PHP_EOL;
