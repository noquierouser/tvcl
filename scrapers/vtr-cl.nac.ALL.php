...
$detalles_url = "http://televisionvtr.cl/index.php?obt=minificha&channels=%CHANNELID%&programs=%PROGRAMID%&starttime=%STARTTIME%&startdate=%STARTDATE%&canal_tipo=Nacional&_=" . time();
...
foreach ($dict_fechas as $dia) {
  ...
  foreach ($grilla as $programa) {
    if ($dict_canales[$programa->parent->{"data-chn"}]) {
      // program data
